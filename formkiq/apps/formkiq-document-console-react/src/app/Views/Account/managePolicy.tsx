...
export function AccessControl() {
  ...
  const [isCreatePolicyModalOpen, setIsCreatePolicyModalOpen] =
    useState<boolean>(false);
  const [isEditPolicyModalOpen, setIsEditPolicyModalOpen] =
    useState<boolean>(false);
  ...
  function onDeletePolicyItem(index: number) {
    function deletePolicyItem() {
      ...
      if (newPolicyItems.length > 0) {
        ...
        DocumentsService.deleteOpenPolicyAgentPolicyItems(siteId).then(
          (response) => {
            if (response.status === 200) {
              updatePolicyItems();
            } else {
            ...
            <div className="flex justify-between items-center px-2 my-2 h-8 gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => onDeletePolicyItem(index)}
                >
                  <Trash />
                </button>
              </div>
