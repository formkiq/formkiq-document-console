...
export function ConfirmRegistration(
  ...
  const onSubmit = async (data: any) => {
    ...
    await formkiqClient.documentsApi.apiClient.cognitoClient.confirmRegistration(props.authApi + '/respondToAuthChallenge', username, userStatus, session, data.password).then((response: any) => {
      if (response.message && response.message === 'Change Password') {
        dispatch(openDialog({ dialogTitle: 'Your password has been saved.'}))
        navigate('/sign-in')
      } else {
            ...
            <div className="md:flex md:items-center mx-4 mb-4 relative">
              ...
              <div className="md:w-2/3 lg:w-1/2">
                ...
                {errors['password'] && errors['password'].message && (
                  <p className="pt-1 text-red-600">{(errors['password'].message as string)}</p>
                )}
              </div>
            </div>
            <div className="md:flex md:items-center mx-4 mb-4 relative">
              <div className="md:w-1/3">
                <label className="block text-gray-500 font-bold md:text-right mb-1 md:mb-0 pr-4" htmlFor="passwordConfirm">
                  Repeat New Password
                </label>
              </div>
              <div className="md:w-2/3 lg:w-1/2">
