...
export function DocumentView(props: { user: User, formkiqVersion: any }) {
    ...
    if (id) {
      ...
      DocumentsService.getDocumentById(id, currentSiteId).then((response: IDocument) => {
        ...
        if (
          ...
          DocumentsService.editDocumentWithOnlyoffice(id, currentSiteId).then((onlyofficeResponse: any) => {
        ...
        } else if (InlineViewableContentTypes.indexOf((response as IDocument).contentType) > -1) {
          let viewVersionKey = ''
  ...
  const [documentContent, setDocumentContent] : [string | null, any] = useState('')
