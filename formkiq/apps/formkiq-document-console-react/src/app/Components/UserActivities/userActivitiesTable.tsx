...
function UserActivitiesTable({
  ...
  useEffect(() => {
    if (userActivities && userActivities.length > 0) {
      ...
      const userIds = userActivities.map((activity: UserActivity) => activity.userId);
      const uniqueUserIds = userIds.filter((value, index, self) => self.indexOf(value) === index);
  ...
  useEffect(() => {
    if (userActivities && userActivities.length > 0) {
      ...
      const documentIds = userActivities.map((activity: UserActivity) => activity.activityId);
      const uniqueDocumentIds = documentIds.filter((value, index, self) => self.indexOf(value) === index);
              ...
              >
                <td className="p-4 pl-8 truncate">
                  {documents[activity.activityId]?.path ?
                    ...
                    >
                      <img
                        src={getFileIcon(documents[activity.activityId].path)}
                ...
                <td className="p-4 ">
                  {formatDate(activity.insertedDate)}
                </td>
