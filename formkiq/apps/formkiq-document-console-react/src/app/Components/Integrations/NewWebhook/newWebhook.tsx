...
export default function NewWebhookModal({isOpened, onClose, updateWebhookExpansion, siteId}: {isOpened: boolean, onClose: any, updateWebhookExpansion: any, siteId: string}) {
    ...
    const onNewFormSubmit = (event: any) => {
      if (formActive && newFormRef.current) {
        ...
        if (!nameValue || typeof(nameValue) === 'object' || !nameValue.length) {
          dispatch(openDialog({ dialogTitle: 'You must provide a name for the webhook'}))
          event.preventDefault();
          return;
        }
        closeDialog();
        DocumentsService.addWebhook(nameValue, siteId).then((data) => {
          // TODO: error handling
          updateWebhookExpansion(siteId)
        })
      }
      event.preventDefault();
    }
                      ...
                      <div className="bg-white p-4 rounded-lg bg-white shadow-xl border h-full">
                        ...
                        <div className="flex flext-wrap mt-4">
                          <form
                            ...
                            <div className="mx-4">
                              <input type="submit" value="Create" className="bg-coreOrange-500 hover:bg-coreOrange-600 text-white font-semibold py-2 px-4 rounded mr-2" />
