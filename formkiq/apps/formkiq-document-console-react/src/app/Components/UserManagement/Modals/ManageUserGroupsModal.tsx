...
import {
  addUserToGroup, deleteUserFromGroup,
  fetchGroups,
...
function ManageUserGroupsModal({isOpen, setIsOpen, userId}: ManageUserGroupsModalPropsType) {
              ...
              >
                <table className="table-auto text-neutral-900 text-sm w-full ">
                  ...
                  <tr>
                    <th scope="col">
                      <div
                        className="px-2 text-start text-transparent bg-clip-text bg-gradient-to-l from-primary-500 via-secondary-500 to-primary-600">
                        Name
                      </div>
                    </th>
                    <th scope="col">
                  ...
                  {groups.length > 0 ? (
                    groups.map((item: Group, index: number) => (
                      ...
                      >
                        <td className="px-2 py-2 border-b border-neutral-300">
                          <Link to={"/admin/groups/" + item.name} title={item.name} target="_blank"
                                rel="noopener noreferrer">
                            <p
                              className="truncate w-32 hover:text-primary-500">
                              {item.name}
                            </p>
                          </Link>
                        </td>
                    ...
                    <tr className="h-[76px]">
                      <td colSpan={7} className="text-center">
                        No groups found. Create a group <Link to="/admin/groups"
                                                              className="text-primary-500 hover:text-primary-600 underline"> here</Link>.
                      </td>
                    </tr>
                  )}
                  </tbody>
