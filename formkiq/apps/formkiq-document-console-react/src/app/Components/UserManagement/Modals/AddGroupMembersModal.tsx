...
import {addUserToGroup, fetchUsers} from "../../../Store/reducers/userManagement";
...
function AddGroupMembersModal({isOpen, setIsOpen, groupName}: AddGroupMembersModalPropsType) {
          ...
          <div className="w-full max-w-xl bg-white p-6">
            ...
            <form className="flex flex-col gap-4 mt-6" onSubmit={onSubmit}>
              <div className="flex gap-2 w-full items-center">
                ...
                <button type="button" onClick={addUser} title="Add User"
              ...
              <div
                className="flex flex-col gap-2 w-full max-h-32 overflow-y-auto border border-neutral-300 rounded-md p-2">
                {usersToAdd.length === 0 && <div className="text-center">No users selected.</div>}
                {usersToAdd.map((user, index) => (
                  <div key={"user" + index}
                    ...
                    <button type="button" className="w-4 h-4 min-w-4 text-neutral-900 hover:text-red-500"
                            onClick={() => setUsersToAdd(usersToAdd.filter(u => u.username !== user.username))}
                            title="Remove">
                      <Close/>
                    </button>
                  </div>
              ...
              <div className="flex flex-row justify-end gap-4 text-base font-bold h-10">
                <ButtonGhost type="button" onClick={closeModal}> CANCEL </ButtonGhost>
