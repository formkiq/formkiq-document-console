  ...
  return (
    ...
    >
      ...
      <tr>
        ...
        <th
          className=" w-full  border-b border-t p-4 pl-8 py-3 bg-clip-text bg-gradient-to-l from-primary-500 via-secondary-500 to-primary-600">Values
        </th>
      </tr>
      </thead>
      <tbody>
      {compositeKeys.length > 0 ? compositeKeys.map((compositeKey, index) => (
          <tr key={"compositeKey_" + index} className="text-neutral-900 border-neutral-300">
            ...
            <td className="border-b">{compositeKey.key.join(', ')}</td>
          </tr>
        )) :
