          ...
          {values.map((key, index) => (
            <Listbox.Option key={key} value={key} className="h-12 hover:bg-neutral-200 px-6 flex items-center text-xs">
              <div className="relative w-full h-full flex items-center">
                <input type="radio" name="status" value={key} checked={selectedValue === key} readOnly
                       className="absolute left-0 top-0 h-full w-full cursor-pointer opacity-0"/>
                <label className="flex items-center justify-between gap-2 w-full"><span
                  className="block truncate">{titles[index]}</span>
                  <div className="w-4">{selectedValue === key ? <CheckedRadio/> : <UncheckedRadio/>}</div>
                </label>
              </div>
            </Listbox.Option>
          ))}
