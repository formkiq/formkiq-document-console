...
function DocumentListLine({
  ...
  return (
    <tr
      className={`text-sm tracking-tight nodark:bg-gray-800 nodark:border-gray-700`}
      ref={drag}
      style={{ opacity, visibility: isDragging ? 'hidden' : 'inherit' }}
    >
      <td className={`bg-white text-gray-800 table-cell pl-${leftOffset} relative`}>
        {
          folder !== 'deleted' &&
          folder !== 'shared' &&
          folder !== 'recent' &&
          folder !== 'favorites' && (
            <div className={`absolute -mt-2 border-l h-full border-coreOrange-50 ` + (lineSubfolderLevel === pageSubfolderLevel ? 'ml-2' : '-ml-2')}></div>
          )
        }
        <div className="flex w-full justify-start">
          ...
          <Link
            to={`${documentsRootUri}/${file.documentId}`}
