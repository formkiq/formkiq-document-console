...
import { Close, Spinner, FolderSolid, ArrowBottom, ArrowRight } from '../../Icons/icons'
import { openDialog } from "../../../Store/reducers/globalNotificationControls"

export default function MoveModal({isOpened, onClose, siteId, value, allTags}: {isOpened: boolean, onClose: any, siteId: string, value: ILine | null, allTags: any}) {
  ...
  const getSubfolders = async (folderUri = '', indexKey = '') => {
    await DocumentsService.getDocumentsInFolder(folderUri, siteId, null, null, null, 50, allTags).then((response: any) => {
      const allFolders = [...[folders]];
      if (response) {
        const retrievedFolders: any[] = response.documents.filter((doc: any) => {
          if (doc.folder) {
            return true
          }
          return false
  ...
  const toggleFolderExpand = (event: any, folderUri: string, indexKey: string) => {
  ...
  const folderLine = (folder: any, index: number, folderUri = '') => {
    ...
    return (
      <div key={index} className={'w-full flex flex-wrap items-center ml-' + (folderUri.split('/').length * 2)}>
        <div className="w-full flex items-center mb-1">
          <div
            className="p-1 cursor-pointer hover:text-gray-400"
            onClick={event => toggleFolderExpand(event, folderUri, folder.indexKey)}
            >
          ...
          <div
            className={'flex py-1 px-2 cursor-pointer active:bg-gray-600 active:text-white ' + (selectedFolderIndexKey === folderUri ? 'bg-gray-600 text-white hover:bg-gray-600 hover:text-white' : 'text-inherit hover:bg-gray-200 hover:text-inherit')}
        ...
        {expandedFolders.indexOf(folder.indexKey) > -1 && (folders as any)['f-' + folder.indexKey] && (
          <div className="w-full mb-2">
            {(folders as any)['f-' + folder.indexKey].length ? (
              <>
                {((folders as any)['f-' + folder.indexKey]).map((subfolder: any, j: number) => {
                  return (
                    folderLine(subfolder, j, folderUri)
                  )
                })}
              </>
            ) : (
  ...
  const executeMove = ( event: any ) => {
    if (value && selectedFolderIndexKey.length) {
      ...
      if (pathOnly === newFolderIndexKey) {
        dispatch(openDialog({ dialogTitle: 'You must select a new folder, not the current one that contains this item.'}))
      } else {
        DocumentsService.setDocumentFolder(newFolderIndexKey, path, siteId).then((res: any) => {
          if (res.status === 400) {
                  ...
                  <Dialog.Panel className="relative transform overflow-hidden text-left transition-all w-full lg:w-1/2 h-1/2">
                    <div className="bg-white p-4 rounded-lg bg-white shadow-xl border h-full">
                      ...
                      <div className="flex flex-wrap mt-4">
                        <div
                          className={'flex py-1 px-2 cursor-pointer active:bg-gray-600 active:text-white ' + (selectedFolderIndexKey === '/root/' ? 'bg-gray-600 text-white hover:bg-gray-600 hover:text-white' : 'text-inherit hover:bg-gray-200 hover:text-inherit')}
                        ...
                        {(folders['root']).length > 0 ? (
                          <>
                            ...
                            <div className="w-full flex justify-center mt-4">
                              <button
                                className={'font-semibold py-2 px-4 mr-2 rounded-2xl ' + (selectedFolderIndexKey.length ? 'cursor-pointer bg-gradient-to-l from-coreOrange-400 via-red-400 to-coreOrange-500 hover:from-coreOrange-500 hover:via-red-500 hover:to-coreOrange-600 text-white' : 'cursor-default bg-gray-200 text-gray-400') }
                              ...
                              <button onClick={closeDialog} className="bg-gradient-to-l from-gray-200 via-stone-200 to-gray-300 hover:from-gray-300 hover:via-stone-300 hover:to-gray-400 text-gray-900 text-base font-semibold py-2 px-5 rounded-2xl flex cursor-pointer focus:outline-none">
                                Cancel
                              </button>
                            </div>
                          </>
