...
export default function RenameModal({isOpened, onClose, siteId, value}: {isOpened: boolean, onClose: any, siteId: string, value: ILine | null}) {
      ...
      if (isOpened) {
        ...
        if (value) {
          ...
          if (value.lineType === 'folder') {
            currentName = value.folder.substring(value.folder.lastIndexOf('/') + 1)
            ...
            setTimeout(() => {
              ...
              if (nameInput) {
                if (nameInput.value.length > 0) {
                  const nameWithoutExtension = nameInput.value.substring(0, nameInput.value.lastIndexOf('.'))
                  nameInput.focus()
                  nameInput.setSelectionRange(0, nameWithoutExtension.length, "forward")
                }
              }
            }, 300)
          }
    ...
    const onRenameFormSubmit = (event: any, value: ILine | null) => {
      if (formActive && value && renameFormRef.current) {
        ...
        if (!nameValue || typeof(nameValue) === 'object' || !nameValue.length) {
          dispatch(openDialog({ dialogTitle: 'You must provide a new name for the item'}))
        ...
        if (value.lineType === 'folder') {
          dispatch(openDialog({ dialogTitle: 'Folder rename has not been implemented yet.'}))
          /*
          // TODO: add site id
          ...
          */
          ...
          if (value && value.documentInstance) {
            DocumentsService.renameDocument(value.documentId, value.documentInstance.path, nameValue, siteId).then((data) => {
                        ...
                        <div className="flex flext-wrap mt-4">
                          <form
                                  ...
                                  { value.lineType === 'folder' ? (
                                    <span>
                                      { value.folder && value.folder.length && (
                                        value.folder.substring(0, value.folder.lastIndexOf('/') + 1)
                                      )}
                                    </span>
                                  ) : (
                                    <span>
                            ...
                            <div className="w-full flex justify-center">
                              <input type="submit" value="Rename" className="bg-gradient-to-l from-coreOrange-400 via-red-400 to-coreOrange-500 hover:from-coreOrange-500 hover:via-red-500 hover:to-coreOrange-600 text-white text-base font-semibold py-2 px-8 rounded-2xl flex cursor-pointer focus:outline-none mr-2" />
