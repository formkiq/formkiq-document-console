...
function MetadataTab({ value, siteId, onDocumentDataChange }: any) {
  ...
  const onMetadataEdit = (fieldName: string, val: any) => {
    if (fieldName === 'deepLinkPath') {
      DocumentsService.patchDocumentDetails(
      ...
      <div className="w-full mt-2 mx-2">
        <table className="border border-neutral-300 border-collapse table-fixed w-full text-sm">
          ...
          <tbody className="bg-white nodark:bg-slate-800">
              ...
              <td className="p-4 text-start">
                ...
                {isEditingDeeplinkPath ? (
                  <div className="flex items-start gap-2">
                    ...
                    />
                    <ButtonPrimaryGradient onClick={handleSaveDeeplinkPath}>
                      Save
                    </ButtonPrimaryGradient>
                    <ButtonGhost onClick={cancelDeeplinkPathEditing}>
                      Cancel
                    </ButtonGhost>
                  </div>
                ) : (
                  <div className="items-start gap-2 font-normal flex max-w-full overflow-hidden w-full">
                    {metadata?.deepLinkPath ? (
                      <p className="max-w-32 truncate mr-2 ">
                        {' '}
                        {metadata?.deepLinkPath}{' '}
                      </p>
                    ) : (
                    ...
                    <div>
                      <button
                        type="button"
                        className="h-4 min-w-4 mt-1 w-4 grow"
                        onClick={startEditingDeeplinkPath}
                      >
                        <Pencil />
                      </button>
                    </div>
            ...
            <tr>
              <td colSpan={2} className="pl-4 text-sm italic">
                NOTE: deeplinks override any existing file object; you can
                restore access to any existing file object by deleting the
                deeplink
              </td>
            </tr>
            <tr className="border-t border-neutral-300">
