...
import { FolderSolid, Search, Spinner, MoreActions } from '../../Icons/icons'
...
export default function SearchInput({onChange, onKeyDown, siteId, documentsRootUri, brand, formkiqVersion, useAdvancedSearch, value, allTags}: any) {

  const [expanded, setExpanded] = useState(false)
  const [documents, setDocuments] = useState(null)
  const [isAdvancedSearchModalOpened, setAdvancedSearchModalOpened] = useState(false);
  ...
  useEffect(() => {
    if (value) {
      ...
      DocumentsService.searchDocuments(siteId, formkiqVersion, null, value, 1, allTags).then((response: any) => {
        if (response?.documents) {
          const temp: any = []
          response.documents?.forEach((el: IDocument) => {
            if (el.documentId) {
              el.insertedDate = moment(el.insertedDate).format('YYYY-MM-DD HH:mm')
              el.lastModifiedDate = moment(el.lastModifiedDate).format('YYYY-MM-DD HH:mm')
              temp.push(el)
            }
          })
          setDocuments(temp)
  ...
  const documentLine = (file: any, i: number) => {
    return (
      <li className="flex items-center p-4 hover:bg-gray-100 cursor-pointer" key={i} >
        <Link to={`${documentsRootUri}/${file.documentId}/view`} className="cursor-pointer w-full" onClick={close}>
            <div className={'fex flex-col'}>
                <div className="inline-block w-8 mr-2 align-text-bottom">
                    <img src={getFileIcon(file.path)} className="w-8 mr-2 inline-block" alt="icon" />
                </div>
  ...
  return (
    <div className={'relative w-full'} ref={wrapperRef} >
      <div className="flex items-center">
        <div _ngcontent-wxp-c51="" className="grow md:flex md:items-center rounded-md mt-2 mb-4 mt-2.5 relative bg-gray-100 " >
        ...
        <div className="grow-0 ml-2 -mt-1">
          { useAdvancedSearch && (
            <button
              ...
              onClick={(event) =>
                onAdvancedSearchModalClick(event)
              }
              >
              <div className="w-4">
                <MoreActions />
      ...
      { expanded && (
        <div className='w-full absolute -mt-2 z-30 bg-white border shadow-xl'>
          <ul className="max-h-[18.375rem] overflow-y-auto rounded-b-lg bg-white text-sm leading-6" role="listbox" id="headlessui-combobox-options-651">
            { useAdvancedSearch && (
              <>
                <li
                  ...
                  > 
                    <span className={'mr-2'}><Search></Search></span>
                    <span className="whitespace-nowrap text-base text-slate-900"> Search folders for { value }</span>
                </li>
                <li
                  className="flex items-center p-4 hover:bg-gray-100 cursor-pointer"
                  ...
                  >
                    <span className={'mr-2'}><Search></Search></span>
                    <span className="whitespace-nowrap text-base text-slate-900"> Search files for { value }</span>
                </li>
              </>
            )}
            <DocumentsPreview />
