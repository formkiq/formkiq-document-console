      ...
      if (formActive && value && newFormRef.current) {
        if (itemToCreate.length) {
          ...
          if (!nameValue || typeof(nameValue) === 'object' || !nameValue.length) {
            dispatch(openDialog({ dialogTitle: 'You must provide a name for the item'}))
          ...
          if (itemToCreate === 'folder') {
            DocumentsService.createFolder((value.folder), nameValue, siteId).then((data) => {
              closeDialog();
            })
          } else {
            ...
            navigate('/documents/new/' + itemToCreate + '?path=' + value.folder + '/' + nameValue)
            closeDialog();
          }
        } else {
          dispatch(openDialog({ dialogTitle: 'You must select the type of item to create'}))
        }
      }
      event.preventDefault();
    }
