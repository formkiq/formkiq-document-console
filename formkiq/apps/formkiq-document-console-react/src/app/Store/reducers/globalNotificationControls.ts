...
export const notificationAction = createAsyncThunk("globalNotificationControls/notificationAction", async (_, thunkAPI) => {
  const { callback } = (thunkAPI.getState() as any)?.globalNotificationControls?.notificationDialog
  callback()
  thunkAPI.dispatch(closeDialog())
  return ''
})

export const closeDialog = createAsyncThunk("globalNotificationControls/closeDialog", async (_, thunkAPI) => {
...
export const { openDialog, hideDialog, resetDialog } = globalNotificationControls.actions
