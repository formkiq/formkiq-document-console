...
import { setDocuments, addDocumentTag, removeDocumentTag, updateDocumentsList } from './documentsList'
import documentsReducer from './documentsList'
...
test('should remove and add value from the document tag and update it in list', () => {
    ...
    let data = documentsReducer(previousState, addDocumentTag({ doc: file, tagKey: 'myTestTag', valueToAdd: 'myTestValue' }))
    expect(data.documents[0].tags).toEqual(
        { favoritedBy:"regan@formkiq.com", myTestTag: 'myTestValue' },
    )

    file = data.documents[0]
    data = documentsReducer(previousState, addDocumentTag({ doc: file, tagKey: 'myTestTag', valueToAdd: 'myTestValue2' }))
    expect(data.documents[0].tags).toEqual(
        { favoritedBy:"regan@formkiq.com", myTestTag: ['myTestValue', 'myTestValue2'] },
    )

    file = data.documents[0]
    data = documentsReducer(previousState, removeDocumentTag({ doc: file, tagKey: 'myTestTag', valueToRemove: 'myTestValue2' }))
    expect(data.documents[0].tags).toEqual(
        { favoritedBy:"regan@formkiq.com", myTestTag: 'myTestValue' },
    )

    file = data.documents[0]
    data = documentsReducer(previousState, removeDocumentTag({ doc: file, tagKey: 'myTestTag', valueToRemove: 'myTestValue' }))
    expect(data.documents[0].tags).toEqual(
        { favoritedBy:"regan@formkiq.com" },
    )
})
